...
\newcommand{\outDate}{Wednesday, October 9}
\newcommand{\dueDate}{Sunday, October 27}
...
$$\boldsymbol{\beta}=
    ...
    \begin{subparts}
        ...
        \CorrectChoice B) $\betav$ with just one row with non-zeros values
...
%\includegraphics[width=8cm, height=4.5cm]{../Q3_1_res.png}
\newline
\begin{minipage}{1\linewidth}
	\centering
	\includegraphics[width=12cm, height=8cm]{../Q3_1_res.png}
	%\refstepcounter{figure}\\  % Increment the figure counter
	%\textbf{Figure \ref{fig:Q3_1_res}:} Training Loss and Verification Loss vs Different Hidden Units  % Manually add a caption/title
	\label{fig:Q3_1_res}         % Label for referencing	
\end{minipage}
\newline
...
\begin{minipage}{1\linewidth}
	\centering
	\includegraphics[width=12cm, height=8cm]{../Q3_2_res_0.03.png}
	%\refstepcounter{figure}\\  % Increment the figure counter
	%\textbf{Figure \ref{fig:Q3_1_res}:} Training Loss and Verification Loss vs Different Hidden Units  % Manually add a caption/title
	\label{fig:Q3_3_res_0.03}         % Label for referencing	
\end{minipage}
\newline
\end{your_solution}
...
\begin{minipage}{1\linewidth}
	...
	%\refstepcounter{figure}\\  % Increment the figure counter
	%\textbf{Figure \ref{fig:Q3_1_res}:} Training Loss and Verification Loss vs Different Hidden Units  % Manually add a caption/title
	\label{fig:Q3_3_res_0.003}         % Label for referencing	
\end{minipage}
\newline
\end{your_solution}
...
\begin{minipage}{1\linewidth}
	...
	%\refstepcounter{figure}\\  % Increment the figure counter
	%\textbf{Figure \ref{fig:Q3_1_res}:} Training Loss and Verification Loss vs Different Hidden Units  % Manually add a caption/title
	\label{fig:Q3_3_res_0.0003}         % Label for referencing	
\end{minipage}
\end{your_solution}
...
% YOUR ANSWER 
\small
Apart from the stating in the above question (b) mentioning that learning rate set to 0.0003 has the slow convergence problem, the case that sets learning rate to 0.03 has the problem of overfitting, as we can see the validation loss keeps increasing, while the training loss is stable.
\end{your_solution}
...
\begin{your_solution}[title=Answer,height=5cm,width=14.5cm]
% YOUR ANSWER 
The values of weight across over rows in $\alpha$ are identical, and the values of weight in $\beta$ across columns, except the first column (bias term), are identical. The reason behind this behavior is the use of zero initialization, making the neurons in a layer produce same output and receive the same gradients. It would have bad results on learning since each neuron intends to capture same pattern in the data, limiting the model's ability to learn versatile features.
\end{your_solution}
\end{subparts}
\clearpage
...
\begin{minipage}{1\linewidth}
	...
	%\refstepcounter{figure}\\  % Increment the figure counter
	%\textbf{Figure \ref{fig:Q3_1_res}:} Training Loss and Verification Loss vs Different Hidden Units  % Manually add a caption/title
	\label{fig:Q3_4_res}         % Label for referencing	
\end{minipage}
\end{your_solution}
