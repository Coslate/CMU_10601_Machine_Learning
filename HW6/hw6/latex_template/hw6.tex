...
\newcommand{\hwName}{Homework 6: Learning Theory, Probabilistic Learning, Fairness Metrics, and Societal Impact}
\newcommand{\dueDate}{Saturday, November 2nd}
...
\begin{parts}
    ...
    \begin{your_solution}[height=4cm]
        % YOUR ANSWER 
        $\hat{R} + O\left(\sqrt{\frac{1}{N} \log\frac{1}{\delta}}\right)$
    \end{your_solution}
...
\part[3] Consider the hypothesis space of functions that map $M$ binary attributes to a binary label. A function $f$ in this space can be characterized as $f: \{0,1\}^M \to \{0,1\}$. Neural the Narwhal says that regardless of the value of $M$, a hypothesis class containing all possible functions in this space can always shatter $2^M$ points. Is Neural wrong? If so, provide a counterexample. If Neural is right, briefly explain why in 1-2 \emph{concise} sentences.\\
    \begin{your_solution}[height=4cm]
        % YOUR ANSWER 
        Neural is right, since the number of all possible functions that maps M binary attributes to a binary label is $2^{2^M}$, which is indeed sufficient to classify all the labeling of $2^{M}$ points, which is also $2^{2^M}$. Hence the hypothesis class containing all these functions always shatter $2^{M}$ points.
    \end{your_solution}
...
\part[2] \textbf{True or False}: Using your responses to the previous question, we achieve statistical parity between regions A and B. Justify your answer. 
    ...
    %YOUR ANSWER
    The selection rate (portion of positive prediction instance) of region A is 70\%, while the selection rate of region B is only 30\%. So, we do not achieve statistical parity between region A and B.
    \end{your_solution}
...
\part[2] \textbf{True or False}: We achieve equality of accuracy between regions A and B. Justify your answer. 
    ...
    \begin{your_solution}[width = \linewidth,height=4cm]
    %YOUR ANSWER
    The accuracy of region A is 60\%, and the accuracy of region B is also 60\%. So, we do achieve the equality of accuracy between region A and B.
    \end{your_solution}
...
\part[2] \textbf{True or False}: We achieve equality of FPR/FNR between regions A and B. Justify your answer. 
    ...
    \begin{your_solution}[width = \linewidth,height=4cm]
    %YOUR ANSWER
    The FPR of region A is 60\%, and  the FNR of region A is 20\%. On the other hand, the FPR of region B is 20\%, and  the FNR of region B is 60\%. As the result, we do not achiece equality of FPR/FNR between regions A and B. 
    \end{your_solution}
...
\part[2] \textbf{True or False}: We achieve equality of PPV/NPV between regions A and B. Justify your answer. 
    ...
    \begin{your_solution}[width = \linewidth,height=4cm]
    %YOUR ANSWER
    The PPV of region A is $\frac{4}{7}$, and the NPV of region A is $\frac{2}{3}$. On the other hand, the PPV of region B is $\frac{2}{3}$, and the NPV of region B is $\frac{4}{7}$. So, we do not achieve equality of PPV/NPV between regions A and B.
    \end{your_solution}
...
\part[3] Using your responses from the previous questions, comment on the fairness of this model between cities A and B.
    ...
    \begin{your_solution}[width = \linewidth,height=4cm]
    %YOUR ANSWER
    The model does not achieve Independence(statistical parity), Separation (equality of FPR/FNR), and Sufficiency (equality of PPV/NPV). Accordingly, the model is not fair between cities A and B.
    \end{your_solution}
...
\part[3] A Type I error occurs when you erroneously predict a positive label (false positive), and a Type II error is when you erroneously predict a negative label (false negative). Compare and contrast the consequences of making a Type I error and Type II error in this setting. Which would cause more significant consequences?
    ...
    %YOUR ANSWER
    \footnotesize
    Type I error (false positive) means you \textbf{wrongly} give a person the loan. The consequence is that there would be a huge risk that the bank would lose its money, as the person may not be able to repay it. On the other hand, the Type II error (false negative) means that you reject to give loan to an applicant who is actually qualified. The consequence is that the bank may loss the profit from interest payments. I think the false positive one would have more significant consequence because the bank would directly loss its capitals due to unpaid loans, while the false negative would only cause the bank to lose its opportunity to earn more money. However, to the perspective of the applicants, the false negative one would have more significant consequence, as he/she would loss the opportunity to buy a house, start a business or do something they want.
    \end{your_solution}
...
\checkboxchar{$\Box$} \checkedchar{$\blacksquare$}
\begin{checkboxes}
    \CorrectChoice False Negative Rate (FNR) parity
    \CorrectChoice False Positive Rate (FPR) parity
    \CorrectChoice Negative Predictive Value (NPV) parity
    \CorrectChoice Positive Predictive Value (PPV) parity
    \CorrectChoice Error parity 
    \choice Statistical parity (or Selection rate parity)
    \choice None of the above
\end{checkboxes}
...
\begin{checkboxes}
    ...
    \choice Positive Predictive Value (PPV) parity
    \choice Error parity 
    \CorrectChoice Statistical parity (or Selection rate parity)
    \choice None of the above
\end{checkboxes}
...
\begin{your_solution}[width = \linewidth,height=5cm]
    %YOUR ANSWER
    The False Negative Rate (FNR) means the portion of patients that truly need hospital care but be predicted as negative, which may lead to serious consequence on the health of these patients. The disparity of FNR means some groups have higher FNR, meaning that the group may have more people suffer from the FNR disparity bias of the model, causing their risk on not being able to recover from illness due to no hospitalization.
...
\part[2] \textbf{Causes of unfairness:}  Name one potential cause of disparity in false negative rates across the two groups in the above context.
    ...
    %YOUR ANSWER
    \small
    I think one potential cause is the feature that the model use, like age and prior utilization of medical insurance, is insufficient to identify all patients in different groups who need hospitalization. For example, assuming that a model considers a person needs more care if he uses more health insurance, and a young and poor person may not have enough money to have medical insurance, then the model may decide that the person does not need hospitalization, while the person may already be infected and indeed needs health care. On the other hand, a rich family may have constant check-up that misleads the model to decide they need more cares. As the model does not have the ability to discover the truly high-risk group, such as old or lower-income patients, they may be neglected and cause the high False Negative Rate and thus the disparity over groups.
    \end{your_solution}
...
\begin{subparts}
    ...
    \def\xone{60}   % Change me! Coordinate 1 x_1 value
    \def\yone{1}    % Change me! Coordinate 1 x_2 value
...
\begin{subparts}
    ...
    %YOUR ANSWER
    We can verify the identity as the following: 
    \tiny
    \begin{align}
		\frac{r_s}{1-r_s} &= \frac{(TP+FN)}{(FP+TN)}\\
		(1-FNR_s) &= 1-\frac{FN}{(TP+FN)} = \frac{TP}{(TP+FN)} \\
		\frac{(1-PPV_s)}{PPV_s} &= \frac{(1-TP/(TP+FP))}{TP/(TP+FP)} = \frac{FP/(TP+FP)}{TP/(TP+FP)} = \frac{FP}{TP} \\
		\frac{r_s}{1-r_s} \times (1-FNR_s) \times \frac{(1-PPV_s)}{PPV_s} &= \frac{(TP+FN)}{(FP+TN)} \times  \frac{TP}{(TP+FN)} \times \frac{FP}{TP} \\
		&= \frac{FP}{FP+TN} \\
		&= FPR_s
	\end{align}
	\normalsize
	As the result, the identity holds for any s $\in$ \{blue, red\}.
    ...
    \begin{your_solution}[width = \linewidth,height=10cm]
    %YOUR ANSWER
    We can show that expression in part(a) can be rewritten as the following:
    \begin{align}
		(1-r_s) FPR_s &= r_s \times (1-FNR_s) \times \frac{1-PPV_s}{PPV_s} \\
