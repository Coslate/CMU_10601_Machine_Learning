...
def rolling_mean(data, window_size):
    ...
    for i in range(len(data)):
        rolling[i] = np.mean(data[max(0, i - window_size + 1):i + 1])
    return rolling
...
if __name__ == "__main__":
    ...
    # Compute rolling mean for the returns
    rolling_avg = rolling_mean(total_reward_episode, 25)

    # Plot the total rewards and rolling mean
    plt.figure(figsize=(12, 6))
    plt.plot(total_reward_episode, label="Total Reward per Episode", alpha=0.6)
    plt.plot(rolling_avg, label="Rolling Mean (25 Episodes)", linewidth=2, color="orange")

    plt.title("Q-Learning: Total Rewards vs Rolling Mean")
    plt.xlabel("Episode")
